...
test_pSet = LocalLearningModel.pSet
...
llmodel = LocalLearningModel(test_pSet)
with torch.no_grad():
    W_standard = torch.tensor(
        [[-0.3783, -0.1536, -0.0994, 0.0785, -0.0857],
         ...
         [-0.4799, 0.2755, 0.3235, -0.2454, 0.0748]], requires_grad=False)
    x = torch.tensor(
                [[[[0.3245, 0.6256],
                   [0.7344, 0.7498]]],
                  [[[0.1765, 0.4858],
...
class TestLocalLearningModel(unittest.TestCase):
    ...
    def test_forward(self):
        ...
        # Index forward
        truth = torch.zeros(mini_batch_size, test_pSet["hidden_size"], requires_grad=False)
        for mini_batch_idx in range(mini_batch_size):
            for mu in range(test_pSet["hidden_size"]):
                for i in range(test_pSet["in_size"]):
                    truth[mini_batch_idx, mu] += llmodel.W[i, mu]*torch.pow(
                            torch.abs(llmodel.W[i, mu]), test_pSet["p"] - 2
    ...
    def test_train(self):
        ...
        for x_mb in x_flat:
            ...
            for mu in range(test_pSet["hidden_size"]):
                ...
                bracket_WW_mu = 0.0
                for i in range(test_pSet["in_size"]):
                    bracket_Wx_mu += W[i, mu]*torch.pow(
                            torch.abs(W[i, mu]), test_pSet["p"] - 2
                            )*x_mb[i]
                    bracket_WW_mu += W[i, mu]*torch.pow(
                            torch.abs(W[i, mu]), test_pSet["p"] - 2
                            )*W[i, mu]
            ...
            for mu in range(test_pSet["hidden_size"]):
                ...
                for i in range(test_pSet["in_size"]):
                    W[i, mu] += g[mu]*(
                        (test_pSet["R"]**test_pSet["p"])*x_mb[i] - bracket_Wx_mu*W[i, mu]
                        ) / test_pSet["tau_l"]
